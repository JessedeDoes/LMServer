...
defaultTextProcessing()
{
   ...
   for f in `perl -pe 's/^/tokenized-trans\/Reichsgericht./'  train.lst | perl -pe 's/([\(\)])/\\\\$1/g' `; do ls ${f}*;  done > Train-lines.lst
   ...
   Create_HMM_list.sh Train-text Reichsgericht-HMMs.lst 6

   # The input of this script is the training text (Train-text), the name of the output file (Reichsgericht-HMMs.lst)
  ...
  Create_MLF.sh Train-lines.lst Reichsgericht.mlf

  Create_DIC_fromconf.sh Reichsgericht.conf
  Create_LM_fromconf.sh Reichsgericht.conf
}
...
otherTextProcessing()
{
  ...
  TRAINING_LINES=/home/jesse/TUTORIAL-HTR/EXP-REICH/TRAIN/Train-Lines.lst
  BUILD_DIRECTORY=/home/jesse/TUTORIAL-HTR/EXP-REICH/TRAIN/LM/PREBUILD

  TEST_PARTITION=/home/jesse/TUTORIAL-HTR/corpora/Reichsgericht/test.noprefix.lst
  TEST_LINES=/home/jesse/TUTORIAL-HTR/EXP-REICH/TRAIN/Test-Lines.lst
...
#  CLASS_CHARSET=eu.transcriptorium.lm.charsets.SyllableTokenization
  ...
  # DICTIONARY=Reichsgericht.dic
  # LANGUAGE_MODEL=Reichsgericht-LM
  # some copying to comply with the other variant

#  cp $LM_OUTPUT/latticeFile.txt EXP-REICH/TRAIN/Reichsgericht-LM.slf
#  cp $LM_OUTPUT/dictionary.txt EXP-REICH/TRAIN/Reichsgericht.dic
#  cp $HMM_LIST EXP-REICH/TRAIN/Reichsgericht-HMMs.lst
#  cp $BUILD_DIRECTORY/labelFile.mlf EXP-REICH/TRAIN/Reichsgericht.mlf
}
...
# the name of the output file (Reichsgericht.mlf)
# and optionally a file containing the correspondence between special character sequences or simbols.
...
# A new column with the number of states has to be added to the Reichsgericht-HMMs.lst file
#echo "../Scripts/HMM-Train_parallel_var.sh Train-lines-feas.lst hmms/ Reichsgericht.mlf  Reichsgericht-HMMs-NumStates.lst 32 64" |qsub -l h_vmem=1g,h_rt=144:00:00 -cwd -N "REIG" -o eixida -j y


HMM-Train_parallel_var.sh Reichsgericht.conf

#or 

#echo "$AbsolutePath/HMM-Train_parallel_var.sh Reichsgericht.conf" |qsub -l h_vmem=1g,h_rt=144:00:00 -cwd -N "REIG" -o eixida -j y
...
#Make the dictionary
#Create_DIC.sh Train-text Reichsgericht.dic [<Transcription-HMM Correspondence file>]
#Create_DIC_fromconf.sh Reichsgericht.conf

# The input of this script is the training text (Train-text), the name of the output file (Reichsgericht.dic)
# and optionally a file containing the correspondence between special character sequences or simbols.
...
# As output we will obtain the language model in ARPA (Reichsgericht-LM.arpa) and in HTK format (Reichsgericht-LM.slf).
