...
#ln -s ../TRAIN/$TEST_DIC
ln -s ../TRAIN/Reichsgericht-HMMs.lst
#ln -s ../TRAIN/$TEST_SLF

# Filter-out paths and extensions from the list of page IDs
for f in `sed -e 's/^/Features\//'  -e 's/.jpg/\*.fea/g'  ../../corpora/Reichsgericht/partitions/test.noprefix.lst`; do ls ${f}*;  done > Test-List

# Launch recognition step parallelized among x cores
Recog-WGGen.sh Reichsgericht.conf


# Replace the WG words by the corresponding real pronunciation
#PutRealPronInWG.sh Reichsgericht.dic WG_REC WG_REC_ED
