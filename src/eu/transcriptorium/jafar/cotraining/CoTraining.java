...
public class CoTraining
{
	...
	public static void main(String[] args) throws IOException
	{
		...
		while (Iter < Iterations && T < Thresholds)
		{
			...
			File dir = new File(
					"/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/CleanedAndNormalized");
			...
			} else
			{
				...
				System.out.println("children.length= " + children.length
						+ " (int)children.length/4 ="
						+ ((int) children.length / 4 + 1)
						+ " (int) children.length/2 " + (int) children.length
						/ 2 + " (int) (3*children.length)/4"
						+ (int) (3 * children.length) / 4);

				T1.firstIndex = 0;
				T1.lastIndex = (int) children.length / 4;
				T1.FileList = children;
				T1.Model = Model;
				T1.Output = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/In/";
				T1.OutputType = "ppl_in_";

				T2.firstIndex = (int) children.length / 4 + 1;
				T2.lastIndex = (int) children.length / 2;
				T2.FileList = children;
				T2.Model = Model;
				T2.Output = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/In/";
				T2.OutputType = "ppl_in_";

				T3.firstIndex = (int) children.length / 2 + 1;
				T3.lastIndex = (int) (3 * children.length) / 4;
				T3.FileList = children;
				T3.Model = Model;
				T3.Output = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/In/";
				T3.OutputType = "ppl_in_";

				T4.firstIndex = (int) (3 * children.length) / 4 + 1;
				T4.lastIndex = (int) children.length - 1;
				T4.FileList = children;
				T4.Model = Model;
				T4.Output = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/In/";
				T4.OutputType = "ppl_in_";

				Model = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/OutBentham";
				OutT1.firstIndex = 0;
				OutT1.lastIndex = (int) children.length / 8;
				OutT1.FileList = children;
				OutT1.Model = Model;
				OutT1.Output = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/Out/";
				OutT1.OutputType = "ppl_out_";

				OutT2.firstIndex = (int) children.length / 8 + 1;
				OutT2.lastIndex = (int) children.length / 4;
				OutT2.FileList = children;
				OutT2.Model = Model;
				OutT2.Output = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/Out/";
				OutT2.OutputType = "ppl_out_";

				OutT3.firstIndex = (int) children.length / 4 + 1;
				OutT3.lastIndex = (int) (3 * children.length) / 8;
				OutT3.FileList = children;
				OutT3.Model = Model;
				OutT3.Output = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/Out/";
				OutT3.OutputType = "ppl_out_";

				OutT4.firstIndex = (int) (3 * children.length) / 8 + 1;
				OutT4.lastIndex = (int) (children.length) / 2;
				OutT4.FileList = children;
				OutT4.Model = Model;
				OutT4.Output = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/Out/";
				OutT4.OutputType = "ppl_out_";

				OutT5.firstIndex = (int) (children.length) / 2 + 1;
				OutT5.lastIndex = (int) (5 * children.length) / 8;
				OutT5.FileList = children;
				OutT5.Model = Model;
				OutT5.Output = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/Out/";
				OutT5.OutputType = "ppl_out_";

				OutT6.firstIndex = (5 * children.length) / 8 + 1;
				OutT6.lastIndex = (int) (6 * children.length) / 8;
				OutT6.FileList = children;
				OutT6.Model = Model;
				OutT6.Output = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/Out/";
				OutT6.OutputType = "ppl_out_";

				OutT7.firstIndex = (int) (6 * children.length) / 8 + 1;
				OutT7.lastIndex = (int) (7 * children.length) / 8;
				OutT7.FileList = children;
				OutT7.Model = Model;
				OutT7.Output = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/Out/";
				OutT7.OutputType = "ppl_out_";

				OutT8.firstIndex = (int) (7 * children.length) / 8 + 1;
				OutT8.lastIndex = (int) children.length - 1;
				OutT8.FileList = children;
				OutT8.Model = Model;
				OutT8.Output = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/Out/";
			...
			T = (Double.parseDouble(InRanks.get(Threshould - 1)) + Double
					.parseDouble(OutRanks.get(Threshould - 1))) / 2;

			System.out
					.println("Thersholdddddddddddddddddddddddddddddddddddddddddd  =============== "
							+ T);
			...
			for (int i = 0; i < Threshould; i++)
			{
				...
				// System.out.println("Out ranks "+InRanks.get(i)+"Out domain = "+RelatedFilesNameOut.get(i)+
				// "  In ranks "+OutRanks.get(i)+" In domain= "+RelatedFilesNameIn.get(i)
				// );
			}
			...
			for (int i = 0; i < Threshould; i++)
			{
				...
				// System.out.println("\n General ConfidentOutFileName(j)==  "+ConfidentOutFileName.get(i)+
				// " RelatedFilesNameIn(i)== "+RelatedFilesNameIn.get(i)+"\n");

				if (!ConfidentOutFileName.contains(ConfidentInFileName.get(i)))
				{
					// int
					// j=ConfidentOutFileName.indexOf(RelatedFilesNameIn.get(i));
					System.out
							.println("\n After selection ConfidentOutFileName(j)==  "
									+ RelatedFilesNameIn.get(i)
									+ " RelatedFilesNameIn(i)== "
									+ RelatedFilesNameIn.get(i) + "\n");
					AddingToOut.add(ConfidentInFileName.get(i));
					// System.out.println("Selected  InRanks === "+InRanks.get(i)+"  OuRanks== "+OutRanks.get(i));
					RemoveFromPPLFiles(PathIn, PathOut,
							ConfidentInFileName.get(i));
				}
				if (!ConfidentInFileName.contains(ConfidentOutFileName.get(i)))
				{
					...
					// System.out.println("Selected  InRanks === "+InRanks.get(i)+"  OuRanks== "+OutRanks.get(i));
					RemoveFromPPLFiles(PathIn, PathOut,
							ConfidentOutFileName.get(i));
				}

			}

			// Adding a set of high-confidence predictions to Out-domain sets
			// which has been predicted by LM(in)

			String OutputTextFile = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/SeletedNormalizedText.txt";

			System.out
					.println(" List of files to be added to Out-domain training set");
			...
			AddToTrainingSet(OutTraing, NewCleanedTrainingSet,
					NewNormalizedTrainingSet);
			...
			AddToTrainingSet(InTraing, NewCleanedTrainingSet,
					NewNormalizedTrainingSet);
			...
			Iter++;
			System.out
					.println("Iterationnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn==================="
							+ Iter);
			...
			ArrayList<String> resultingFileName)
	{
		...
		for (String st : resultingFileName)
		{
			...
			st = "/mnt/Projecten/transcriptorium/Tools/languagemodeling/TestSampleSelection/CleanedText"
					+ st;
			File rm = new File(st);
			...
			String outputTextFileOriginal)
	{
		...
		if (nrmcleand.delete())
			System.out
					.println(" The Selected Normalized Text in this iteration has benn removed !!!!!");

		File rmcleand = new File(outputTextFileOriginal);
		if (rmcleand.delete())
			System.out
					.println(" The Selected Cleaned Text in this iteration has benn removed !!!!!");
			...
			String FileNameForDelete)
	{
		...
		String st = FileNameForDelete.substring(0,
				FileNameForDelete.lastIndexOf(":"));
		...
		{
			System.out
					.println("errooooooooooooooooooooooooooooooooooooooooooooooooorrrrrrrrrrr"
							+ Inst);
		}
	...
	public static void AddToTrainingSet(String TraingFolder,
			String newCleanedTrainingSet, String newNormalizedTrainingSet)
			throws IOException
	{
		// TODO Auto-generated method stub
	...
	private static void RetrivedBestMachesCleanedText(
			ArrayList<String> resultingFileName, String outputTextFile)
			throws IOException
	{
		...
		for (String st : resultingFileName)
		{
			 ...
			 * File rm = new File(st);
			 * 
			 * if(rm.delete()){ System.out.println(rm.getName() +
			 * " is deleted!"); }
			 */// for now do not remove the files
			...
			String outputTextFile) throws IOException
	{
		...
		BufferedWriter outlist = new BufferedWriter(new FileWriter(fileList,
				true));
	...
	public static ArrayList<String> CorrelationFunction(File dir,
			String inputpath, ArrayList<String> FileNames) throws IOException
	{
		// TODO Auto-generated method stub
			...
			for (int i = 0; i < children.length; i++)
			{
				...
				while (line != null)
				{
					...
					if (j == 1)
					{
						...
						OOVs.add((double) (Double.parseDouble(seperate[6]) / Double
								.parseDouble(seperate[4])));
					} else
					{
		 ...
		 * System.out
		 * .println("Correlation between Sentences and Words= "+SentWord_Correlation
		 * ); double Wordppl1_Correlation=Correlationlogppl1(Word,ppl1) ;
		 * System.
		 * out.println("Correlation between Words and ppl1= "+Wordppl1_Correlation
		 * ); double OOVsppl1_Correlation=Correlationlogppl1(OOVs,ppl1) ;
		 * System.
		 * out.println("Correlation between OOVs and ppl1= "+OOVsppl1_Correlation
		 * ); double OOVslogprob_Correlation=Correlationlogppl1(OOVs,Logprobs) ;
		 * System.out.println("Correlation between OOVs and Logprob= "+
		 * OOVslogprob_Correlation); double
		 * Wordlogprob_Correlation=Correlationlogppl1(Word,Logprobs) ;
		 * System.out
		 * .println("Correlation between Word and Logprob= "+Wordlogprob_Correlation
		 * );
		 * 
		 * double PPL1logprob_Correlation=Correlationlogppl1(Logprobs,ppl1) ;
		 * System.out.println("Correlation between Logprob and ppl1 = "+
		 * PPL1logprob_Correlation);
		 */

		for (int i = 0; i < OOVs.size(); i++)
		{
			Result.add(String.valueOf(ppl1.get(i) * OOVs.get(i)) + ","
					+ FileNames.get(i));

			// Result.add(String.valueOf(
			// (double)(1.0/ppl1.get(i))*(1.0-OOVs.get(i)))+","+FileNames.get(i));
	...
	private static double Correlationlogppl1(ArrayList<Double> sent,
