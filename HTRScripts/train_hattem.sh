...
defaultTextProcessing()
{
   ...
   for f in `perl -pe 's/^/tokenized-trans\/Hattem./'  train.lst`; do ls ${f}*;  done > Train-lines.lst
   ...
   Create_HMM_list.sh Train-text Hattem-HMMs.lst 6

   # The input of this script is the training text (Train-text), the name of the output file (Hattem-HMMs.lst)
  ...
  Create_MLF.sh Train-lines.lst Hattem.mlf

  Create_DIC_fromconf.sh Hattem.conf
  Create_LM_fromconf.sh Hattem.conf
}
...
otherTextProcessing()
{
  ...
  # DICTIONARY=Hattem.dic
  # LANGUAGE_MODEL=Hattem-LM
  # some copying to comply with the other variant

  cp $LM_OUTPUT/latticeFile.txt EXP-HATTEM/TRAIN/Hattem-LM.slf
  cp $LM_OUTPUT/dictionary.txt EXP-HATTEM/TRAIN/Hattem.dic
  cp $HMM_LIST EXP-HATTEM/TRAIN/Hattem-HMMs.lst
  cp $BUILD_DIRECTORY/labelFile.mlf EXP-HATTEM/TRAIN/Hattem.mlf
}
...
# the name of the output file (Hattem.mlf)
# and optionally a file containing the correspondence between special character sequences or simbols.
...
# A new column with the number of states has to be added to the Hattem-HMMs.lst file
#echo "../Scripts/HMM-Train_parallel_var.sh Train-lines-feas.lst hmms/ Hattem.mlf  Hattem-HMMs-NumStates.lst 32 64" |qsub -l h_vmem=1g,h_rt=144:00:00 -cwd -N "REIG" -o eixida -j y


HMM-Train_parallel_var.sh Hattem.conf

#or 

#echo "$AbsolutePath/HMM-Train_parallel_var.sh Hattem.conf" |qsub -l h_vmem=1g,h_rt=144:00:00 -cwd -N "REIG" -o eixida -j y
...
#Make the dictionary
#Create_DIC.sh Train-text Hattem.dic [<Transcription-HMM Correspondence file>]
#Create_DIC_fromconf.sh Hattem.conf

# The input of this script is the training text (Train-text), the name of the output file (Hattem.dic)
# and optionally a file containing the correspondence between special character sequences or simbols.
...
# As output we will obtain the language model in ARPA (Hattem-LM.arpa) and in HTK format (Hattem-LM.slf).
