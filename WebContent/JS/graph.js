...
var Graph = Class.extend({
    ...
    setLabel: function(n, l) {
      n.label = l;
      {
          var div = document.createElement('div');
           var txt = ((n.label)+"").replace("\\\"", "\"");
           txt = txt.replace("<","&lt;");
           txt = txt.replace(">","&gt;");
           div.innerHTML = (n.href)? '<a href="'+n.href+'">'+txt+"</a>" : txt;
           div.className = (n.css)? ("node-label " + n.css) : "node-label";
           div.style.fontFamily = (n.font)? n.font : "";
           div.style.fontSize   = (n.fontsize)? n.fontsize+"px" : "";
           div.style.fontWeight = (n.fontweight)? n.fontweight : ""; // XXX doesn't work for "italic" (=fontStyle).
           div.style.color      = (typeof(n.text) == "string")? n.text : "";
           n.text = div;
          _unselectable(div);
        }
    },  
    addNodeWithLabel: function(id, a, labl) {
        /* Appends a new Node to the graph.
         */
        var n = a && a._super || Node;
            n = (id instanceof Node)? id : (this.nodeset[id])? this.nodeset[id] : new n(id, a);
