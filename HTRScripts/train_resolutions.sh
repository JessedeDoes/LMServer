...
defaultTextProcessing()
{
   ...
   for f in `perl -pe 's/^/tokenized-trans\/Resolutions./'  train.lst`; do ls ${f}*;  done > Train-lines.lst
   ...
   Create_HMM_list.sh Train-text Resolutions-HMMs.lst 6

   # The input of this script is the training text (Train-text), the name of the output file (Resolutions-HMMs.lst)
  ...
  Create_MLF.sh Train-lines.lst Resolutions.mlf

  Create_DIC_fromconf.sh Resolutions.conf
  Create_LM_fromconf.sh Resolutions.conf
}
...
otherTextProcessing()
{
  ...
  TRANSCRIPTIONS=/home/jesse/TUTORIAL-HTR/EXP-RESOLUTIONS/TRAIN/Transcriptions
  TRAINING_PARTITION=/home/jesse/TUTORIAL-HTR/EXP-RESOLUTIONS/TRAIN/train.lst
  TRAINING_LINES=/home/jesse/TUTORIAL-HTR/EXP-RESOLUTIONS/TRAIN/Train-Lines.lst
  ...
  LM_OUTPUT=EXP-RESOLUTIONS/TRAIN/LM
  CUTOFF=0
  ...
  # DICTIONARY=Resolutions.dic
  # LANGUAGE_MODEL=Resolutions-LM
  # some copying to comply with the other variant

  cp $LM_OUTPUT/latticeFile.txt EXP-RESOLUTIONS/TRAIN/Resolutions-LM.slf
  cp $LM_OUTPUT/dictionary.txt EXP-RESOLUTIONS/TRAIN/Resolutions.dic
  cp $HMM_LIST EXP-RESOLUTIONS/TRAIN/Resolutions-HMMs.lst
  cp $BUILD_DIRECTORY/labelFile.mlf EXP-RESOLUTIONS/TRAIN/Resolutions.mlf
}
...
# the name of the output file (Resolutions.mlf)
# and optionally a file containing the correspondence between special character sequences or simbols.
...
# A new column with the number of states has to be added to the Resolutions-HMMs.lst file
#echo "../Scripts/HMM-Train_parallel_var.sh Train-lines-feas.lst hmms/ Resolutions.mlf  Resolutions-HMMs-NumStates.lst 32 64" |qsub -l h_vmem=1g,h_rt=144:00:00 -cwd -N "REIG" -o eixida -j y


HMM-Train_parallel_var.sh Resolutions.conf

#or 

#echo "$AbsolutePath/HMM-Train_parallel_var.sh Resolutions.conf" |qsub -l h_vmem=1g,h_rt=144:00:00 -cwd -N "REIG" -o eixida -j y
...
#Make the dictionary
#Create_DIC.sh Train-text Resolutions.dic [<Transcription-HMM Correspondence file>]
#Create_DIC_fromconf.sh Resolutions.conf

# The input of this script is the training text (Train-text), the name of the output file (Resolutions.dic)
# and optionally a file containing the correspondence between special character sequences or simbols.
...
# As output we will obtain the language model in ARPA (Resolutions-LM.arpa) and in HTK format (Resolutions-LM.slf).
